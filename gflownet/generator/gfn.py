...
class TBGFlowNet(nn.Module):
    ...
    def TBLoss(self, s, a, r, d, n, idc, lens):
        ...
        losses = torch.clamp(self.logZ + forward_ll - log_rewards - backward_ll, max=100, min=-100).pow(2)
...
class SubTBGFlowNet(nn.Module):
    ...
    def _compute_subtb_loss_for_trajectory(self, log_pf, log_pb, log_F, log_R, traj_len):
        ...
        if T == 1:
            ...
            loss = torch.clamp(errors, min=-100, max=100).pow(2).sum()
        else:
            ...
            loss = (weights * torch.clamp(subtraj_errors, min=-100, max=100).pow(2)).sum()
        ...
        loss = loss + torch.clamp(full_traj_error,min=-100,max=100).pow(2)
