...
class FMGFlowNet(nn.Module):
    ...
    def FMLoss(self, p, pb, a, r, s, d):
        ...
        term_loss = (f_loss * (d==2)).sum() / ((d==2).sum() + 1e-20)  # terminal nodes
        flow_loss = (f_loss * (d!=2)).sum() / ((d!=2).sum() + 1e-20)  # non-terminal nodes

        # leaky_term_loss = (l_loss * (d!=0)).sum() / ((d!=0).sum() + 1e-20)  # terminal nodes
        # leaky_flow_loss = (l_loss * (d==0)).sum() / ((d==0).sum() + 1e-20)  # non-terminal nodes
        leaky_loss = l_loss.mean()
        # print((d==1).sum(), 'pruned_loss')
        # print((d==2).sum(), 'term_loss')
        # print((d==0).sum(), 'flow_loss')
        if self.balanced_loss:
            # leaky_loss = leaky_term_loss * self.leaf_coef + leaky_flow_loss
            loss = term_loss * self.leaf_coef + flow_loss + leaky_loss * self.pruned_coef
        else:
            # loss = term_loss + flow_loss + leaky_term_loss + leaky_flow_loss
            # loss += pruned_loss
            loss = term_loss + flow_loss + leaky_loss

        return loss, term_loss, flow_loss, leaky_loss
...
class TBGFlowNet(nn.Module):
    ...
    def forward(self, graph_data, vec_data=None, do_stems=True, return_leaky=False):
        """Forward pass with optional leaky output for compatibility with FMGFlowNet."""
        if return_leaky:
            # For compatibility: return same output twice (no leaky version in TB)
