...
from utils.metrics import evaluate, compute_correlation
...
def arg_parse():
    ...
    parser.add_argument("--log_dir", default='../checkpoint/soo')
    ...
    # objectives
    parser.add_argument("--objectives", type=str, default='gsk3b')
    ...
    parser.add_argument("--log_reg_c", default=(0.1/8) ** 4, type=float)  # (0.1/8)**8  
...
class RolloutWorker:
    ...
    def rollout(self, generator, use_rand_policy=True, replay=False):
        ...
        for t in range(max_blocks):
            ...
            if t >= self.min_blocks and action == 0:
                r, raw_r = self._get_reward(m)  # r: reward, raw_r: scores for the objectives
                ...
                if len(m.blocks) and not len(m.stems) or t == max_blocks - 1:
                    ...
                    if self.ignore_parents:
                        ...
                        samples.append((parents, actions, r, m, 1))
                    break
                else:
                    if self.ignore_parents:
        ...
        self.sampled_mols.append(
            ([i.cpu().numpy() for i in raw_r], m, trajectory_stats, inflow))
    ...
    def _get_reward(self, m):
        ...
        assert score.shape == (1, len(self.args.objectives)), f"Score shape: {score.shape}"
        raw_reward = score.sum()
    ...
    def execute_train_episode_batch(self, generator, dataset=None, use_rand_policy=True):
        samples = sum((self.rollout(generator, use_rand_policy)
...
def train_generative_model_with_oracle(args, generator, bpath, oracle, dataset=None, do_save=False):
    ...
    for i in range(args.num_iterations + 1):
        ...
        if multi_thread:
            ...
            p, pb, a, r, s, d, mols = rollout_worker.sample2batch(
                rollout_worker.execute_train_episode_batch(generator, dataset, use_rand_policy=True))

        loss = generator.train_step(p, pb, a, r, s, d, mols, i)
        ...
        if not i % 100:
            ...
            if not i % args.sample_iterations: # and i != 0:
                diversity = evaluate(args, generator, rollout_worker, 100)
...
def main(args):
    ...
    rollout_worker, training_metrics = train_generative_model_with_oracle(
        args, generator, bpath, oracle, do_save=args.save)

    args.logger.save(os.path.join(args.log_dir, 'logged_data.pkl.gz'))

if __name__ == '__main__':
    args = arg_parse()
    args.log_dir = os.path.join(args.log_dir, args.objectives)
