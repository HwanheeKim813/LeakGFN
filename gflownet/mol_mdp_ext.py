...
class BlockMoleculeDataExtended(BlockMoleculeData):
    ...
    def copy(self): # shallow copy
        ...
        o.init_stem = list(self.init_stem) if self.init_stem is not None else None
        return o
...
class MolMDPExtended(MolMDP):
    ...
    def build_translation_table(self):
        ...
        for blockidx, atom_map in self.translation_table.items():
            new_atom_map = defaultdict(list)
            for atom, block_duplicate in atom_map.items():
                new_atom_map[block_duplicate].append(atom)
            self.reverse_translation_table[blockidx] = new_atom_map
        # self.reverse_translation_table = {v: keys for v, keys in reverse_translation_table.items() if len(keys) > 1}
    ...
    def parents(self, mol=None):
        ...
        for rblockidx in blocks_degree_1:
            ...
            # print('new_mol.stems', new_mol.stems)
            # print('mol.blockidxs', mol.blockidxs)
            # print('new_mol.blockidxs', new_mol.blockidxs)
            removed_stem_atom = (rbond[3] if rblockidx == rbond[1] else rbond[2])
            # print('removed_stem_atom', removed_stem_atom)
            ...
            next_block_idx = self.translation_table[blockid][removed_stem_atom]
            # print('next_block_idx', next_block_idx)
            lblockidx = new_mol.stems[-1][0]
            # print('lblockidx', lblockidx)
            lstem_atom = new_mol.stems[-1][1]
            # print('lstem_atom', lstem_atom)
            stem_block_idx = new_mol.blockidxs[lblockidx]
            # print('stem_block_idx', stem_block_idx)
            # print('self.translation_table[stem_block_idx]', self.translation_table[stem_block_idx])
            lstem_block_idx = self.translation_table[stem_block_idx][lstem_atom]
            # print('lstem_block_idx', lstem_block_idx)
            symmetric_duplicate = self.reverse_translation_table[stem_block_idx][lstem_block_idx]
            # print('symmetric_duplicate', symmetric_duplicate)
            default_stem_idx = len(new_mol.stems) - 1
            # print('removed_stem_atom', removed_stem_atom)
            # print('blockid', blockid)
            # print('stem_block_idx', stem_block_idx)
            # print('symmetric_duplicate', symmetric_duplicate)
            # print('lblockidx', lblockidx)
            # print('new_mol.stems', new_mol.stems)
            # print('default_stem_idx', default_stem_idx)
            if len(symmetric_duplicate) > 1 and default_stem_idx > 0:
                ori_smi = mol.smiles
                for sym_atom_idx in symmetric_duplicate:
                    sym_stem = [lblockidx, sym_atom_idx]
                    if sym_stem in new_mol.stems:
                        sym_stem_idx = new_mol.stems.index(sym_stem)
                        sym_mol_smi = self.add_block_to(new_mol, next_block_idx, sym_stem_idx).smiles
                        if ori_smi == sym_mol_smi:
                            parent_mols.append([new_mol,
                                # action = (block_idx, stem_idx)
                                (next_block_idx, sym_stem_idx)])
            else:
                parent_mols.append([new_mol,
                                # action = (block_idx, stem_idx)
                                (next_block_idx, default_stem_idx)])
            
            # parent_mols.append([new_mol,
            #                     # action = (block_idx, stem_idx)
            #                     (r_block_idx, default_stem_idx)])
            # if len(symmetric_duplicate) > 1 and default_stem_idx > 0:
            #     ori_smi = mol.smiles
            #     for sym_atom_idx in symmetric_duplicate:
            #         sym_stem = [l_block_idx, sym_atom_idx]
            #         if sym_stem in new_mol.stems:
            #             sym_stem_idx = new_mol.stems.index(sym_stem)
            #             if sym_stem_idx == default_stem_idx:
            #                 continue
            #             sym_mol_smi = self.add_block_to(new_mol, r_block_idx, sym_stem_idx).smiles
            #             if ori_smi == sym_mol_smi:
            #                 parent_mols.append([new_mol,
            #                     # action = (block_idx, stem_idx)
            #                     (r_block_idx, sym_stem_idx)])
