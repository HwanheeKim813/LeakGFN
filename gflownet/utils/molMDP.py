...
class BlockMoleculeData:
    ...
    def add_block(self, block_idx, block, block_r, stem_idx, atmidx):
        ...
        if len(self.blocks)==1 and self.init_stem is None:
            self.stems.append([self.numblocks-1, block_r[0]])
            self.init_stem = []
            [self.init_stem.append([self.numblocks-1,r]) for r in block_r[1:]]
        # elif self.init_stem is not None:
        #     self.stems += self.init_stem
        #     self.init_stem = None
        #     [self.stems.append([self.numblocks-1,r]) for r in block_r[1:]]
        else:
            if self.init_stem is not None:
                self.stems += self.init_stem
                self.init_stem = None
                assert stem_idx == 0, "stem_idx is not 0"
            [self.stems.append([self.numblocks-1,r]) for r in block_r[1:]]
            if stem_idx is None:
